classdef ESInterface < handle
    ...
    methods
        function obj = ESInterface(varargin)
            ...
            try
                % Ensure /app is on Python sys.path so MATLAB's embedded Python finds the
                % shipped `energy_agent` package (non-invasive; safe to call repeatedly)
                try
                    insert(py.sys.path, int32(0), '/app');
                    fprintf('Prepended /app to Python sys.path for MATLAB-Python import resolution\n');
                catch ME2
                    fprintf('Warning: failed to modify Python sys.path: %s\n', ME2.message);
                end
